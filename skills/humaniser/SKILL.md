---
name: humaniser
# Externally created (adapted from Siqi Chen's "humanizer", MIT — see Reference, bottom). Centralised
# here for management; vendored verbatim — the skill body is unedited. `version` below is UPSTREAM's
# own version, not ours: use it to track which upstream we vendored, and bump it only on re-vendor.
version: 2.8.0
description: |
...
> Adapted from the humanizer skill by Siqi Chen, MIT licensed. Source: https://github.com/blader/humanizer
